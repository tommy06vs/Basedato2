...
public class ConsultaDao {
    ...
    public boolean isIdAlreadyExists(String id) {// si el id ya existe para registrar
...
//puedes realizarme algunas modificaciones adicionales. implementa la accion de registro del metodo: registrarNuevoEstudiante()
//de la clase ConsultaDao, dentro del ActionListener del botton btnVerTabla2 que el usuario pueda ingresar en los respectivos textFiled para completar el registro
    //1) Registrar un nuevo estudiante junto con sus materias inscritas y notas*******************************************************************
    public void registrarNuevoEstudiante(String codigoId, String nombre, String apellidoPaterno, String fechaNacimiento, String carrera,
        ...
        try {
            ...
            ps.executeUpdate();
            logger.info("Se registra un nuevo estudiante junto con sus materias inscritas y notas");
        } catch (SQLException e) {
            logger.error("Error al registrar el nuevo estudiante: " + e.getMessage());
            // Aquí puedes manejar la excepción según tus requerimientos
        }
    }
    public void registrarNuevoEstudiante2(String codigoId, String nombre, String apellidoPaterno, String fechaNacimiento, String carrera,
        ...
        try {
            ...
            // Convertir el String de la fecha de nacimiento a java.sql.Date
            //SimpleDateFormat dateFormat = new SimpleDateFormat("yyyyMMdd");
            //Date parsedDate = (Date) dateFormat.parse(fechaNacimiento);
    ...
    public ListaDoble<MateriaDto> getMateriasPorEstudiante(String codigoIdEstudiante) {
        ...
        String sql = "SELECT e.codigo_id, e.snombre, e.sappaterno, e.sapmaterno, en.materia_id, m.snombre " +
                "FROM estudiantes e " +
        ...
        try {
            ...
            logger.error("Error al obtener las materias del estudiante: " + e.getMessage());
            // Aquí puedes manejar la excepción según tus requerimientos
        }
        return resultado;
    }
    ...
    public void insert(EstudianteDto nuevo){
        if (isIdAlreadyExists(nuevo.getCodigoId())){
...
/*tomando de referencia el anterior codigo implementa los metodo en la clase "ConsultaDao" tomando de referencia
la clase "ConexionPostgreSQL", "ListaDoble<E>", "EstudianteDto", "EstudianteNotaDto", "NotaDto" y "MateriaDto". elimnar un estudiante o una
materia de la base de datos primero
        (--primero
        DELETE FROM notas
        WHERE codigo_id = '000001';
        -- segundo
        INSERT INTO estudiantes (codigo_id, snombre, sappaterno, dtnacimiento, materia_id, carrera)
        VALUES ('000003', 'Juan', 'Perez', '20000115', "m01", 'Carrera Nueva');
        -- tercero
        INSERT INTO Materias (codigo_id, snombre, nro_credito)
        VALUES ('m02', 'contabilidad 1', 5);
        -- cuarto
        insert into notas (codigo_id, eacademico, descripcion, materia_id) values('n01', 'Sobresaliente', null, null);
        -- quinto
        INSERT INTO estudiantesNotas (estudiante_id, nota_id, tnotas, materia_id) VALUES ('252565', 'n01', 100, 'm01');)
es una consulta en PostgreSQL implememtalo en codigo en la clase ConsultaDao.
        -- sexto
        DELETE FROM estudiantesNotas
        WHERE estudiante_id = '000001'
        AND materia_id = 'm01';
puedes tomar de referencia las clases mensionada anterior mente "EstudianteDto", "MateriaDto", "NotaDto"
 y "EstudianteNotaDto" que se hace referencia a sus atributos y las tablas en PostgreSQL:
        CREATE TABLE estudiantes (
        codigo_id varchar not null primary key,
        snombre varchar,
        sappaterno varchar,
        sapmaterno varchar,
        dtnacimiento date,
        ...
        CREATE TABLE estudiantesNotas (
        estudiante_id VARCHAR,
        nota_id VARCHAR,
        tnotas INT,
        materia_id VARCHAR,
        PRIMARY KEY (estudiante_id, nota_id, materia_id),
        FOREIGN KEY (estudiante_id) REFERENCES estudiantes(codigo_id),
        FOREIGN KEY (nota_id) REFERENCES notas(codigo_id),
        FOREIGN KEY (materia_id) REFERENCES materias(codigo_id)
        );
        create table notas(
        codigo_id varchar not null primary key,
        eacademico varchar,--estado academico
        descripcion varchar,
        materia_id varchar
        );
